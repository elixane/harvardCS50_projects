...
NONTERMINALS = """
S -> NP VP | NP VP Conj NP VP | NP VP Conj VP | S Conj S
AP -> Adj | Adj AP
...
def np_chunk(tree):
    ...
    # Traverse all subtrees in the tree
    for subtree in tree.subtrees():
        # Check if the subtree is a noun phrase (NP)
        if subtree.label() == "NP":
            # Check if this NP subtree contains any other NP subtrees
            if not any(child.label() == "NP" for child in subtree.subtrees(lambda t: t != subtree)):
                # If no other NP subtrees, it is an NP chunk
